...
constexpr uint8_t FONT[][5] = {
  ...
  {0x00,0x00,0x00,0x00,0x00}, {0x00,0x00,0x5F,0x00,0x00}, {0x00,0x36,0x36,0x00,0x00},
  {0x40,0x40,0x40,0x40,0x40}, {0x08,0x08,0x08,0x08,0x08}, {0x00,0x60,0x60,0x00,0x00},
  {0x20,0x10,0x08,0x04,0x02}, {0x23,0x13,0x08,0x64,0x62},
};

constexpr int START_X = 145, START_Y = 230, START_W = 190, START_H = 60;
constexpr int YES_X = 75, NO_X = 275, CONFIRM_Y = 235, CONFIRM_W = 130, CONFIRM_H = 55;
}
...
const uint8_t *SdQualificationUi::glyph(char c) const {
  ...
  switch (c) {
    case ' ': return FONT[36]; case '!': return FONT[37]; case ':': return FONT[38];
    case '_': return FONT[39]; case '-': return FONT[40]; case '.': return FONT[41];
    case '/': return FONT[42]; case '%': return FONT[43]; default: return FONT[36];
  }
}
...
void SdQualificationUi::drawCentered(int y, const char *text, uint16_t color, int scale) {
  drawText((board_.display().width() - static_cast<int>(strlen(text)) * 6 * scale) / 2,
           y, text, color, scale);
}
...
void SdQualificationUi::drawButton(const Rect &r, const char *label, uint16_t fill, uint16_t text) {
  ...
  drawText(r.x + (r.w - tw) / 2, r.y + (r.h - 7 * scale) / 2, label, text, scale);
}
...
void SdQualificationUi::drawProgressBar(int x, int y, int w, int h, uint8_t percent) {
  ...
  const int filled = ((w - 10) * percent) / 100;
...
bool SdQualificationUi::released(uint32_t timeoutMs) {
  ...
  while (millis() - start < timeoutMs) {
    WT32_SC01_PLUS_TouchPoint points[2]; uint8_t count = 0;
...
void SdQualificationUi::showReady(const SdCardInfo &info) {
  ...
  snprintf(line, sizeof(line), "CARD: %s", info.name[0] ? info.name : "SD"); drawText(30, 105, line, WHITE, 2);
  snprintf(line, sizeof(line), "SIZE: %llu MIB", static_cast<unsigned long long>(info.bytes / (1024ULL * 1024ULL))); drawText(30, 132, line, WHITE, 2);
  snprintf(line, sizeof(line), "SECTORS: %llu", static_cast<unsigned long long>(info.sectors)); drawText(30, 159, line, WHITE, 2);
  drawText(30, 188, "00 / AA / 55 + VERIFY + FAT", YELLOW, 2);
  drawButton({START_X, START_Y, START_W, START_H}, "START", GREEN, BLACK);
}

bool SdQualificationUi::waitForStart() { return waitForTap({START_X, START_Y, START_W, START_H}); }
...
bool SdQualificationUi::confirmErase() {
  ...
  drawButton(yes, "ERASE", RED, WHITE); drawButton(no, "CANCEL", GRAY, BLACK);
  released();
  for (;;) {
    WT32_SC01_PLUS_TouchPoint points[2]; uint8_t count = 0;
    if (!board_.touch().readPoints(points, 2, count)) return false;
    for (uint8_t i = 0; i < count; ++i) {
      if (pointInRect(points[i].x, points[i].y, yes)) { delay(80); released(); return true; }
      if (pointInRect(points[i].x, points[i].y, no)) { delay(80); released(); return false; }
    }
    delay(20);
  }
}

void SdQualificationUi::showProgress(const char *phase, uint8_t pattern, uint8_t stage,
                                     uint8_t totalStages, uint64_t doneSectors,
                                     uint64_t totalSectors, double rateMiBPerSec,
                                     uint32_t etaSeconds) {
  const uint8_t percent = totalSectors ? static_cast<uint8_t>((doneSectors * 100ULL) / totalSectors) : 0;
  if (stage != lastStage_) { board_.display().fillScreen(BLACK); lastStage_ = stage; lastPercent_ = 0xFF; }
  ...
  snprintf(line, sizeof(line), "STAGE %u / %u", stage, totalStages); drawCentered(58, line, WHITE, 2);
  if (pattern) snprintf(line, sizeof(line), "%s 0X%02X", phase, pattern);
  else snprintf(line, sizeof(line), "%s", phase);
  drawCentered(90, line, YELLOW, 3);
  snprintf(line, sizeof(line), "%u%%", percent); drawCentered(135, line, GREEN, 4);
  drawProgressBar(35, 180, 410, 36, percent);
  snprintf(line, sizeof(line), "RATE %.2f MIB/S", rateMiBPerSec); drawText(45, 235, line, WHITE, 2);
...
void SdQualificationUi::showFailure(const SdQualFailure &failure) {
  ...
  snprintf(line, sizeof(line), "PATTERN 0X%02X", failure.pattern); drawText(20, 115, line, WHITE, 2);
  snprintf(line, sizeof(line), "LBA %llu", static_cast<unsigned long long>(failure.lba)); drawText(20, 145, line, WHITE, 2);
  snprintf(line, sizeof(line), "OFFSET %u", failure.offset); drawText(20, 175, line, WHITE, 2);
  snprintf(line, sizeof(line), "EXPECTED %02X ACTUAL %02X", failure.expected, failure.actual); drawText(20, 205, line, WHITE, 2);
  snprintf(line, sizeof(line), "ERROR %d", failure.error); drawText(20, 235, line, WHITE, 2);
}
