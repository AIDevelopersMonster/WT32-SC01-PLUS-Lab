/*
 * WT32-SC01-PLUS-Lab
 * Example 17: LVGL Settings Persistence
 *
 * Target hardware:
 *   Panlee WT32-SC01-PLUS / ZX3D50CE08S-V15-USRC / 230208
 *
 * Purpose:
 *   Extend the physically validated Light/Dark LVGL HMI with persistent
 *   theme and backlight settings stored in ESP32 NVS via Preferences.
 *
...
constexpr ThemePalette kDarkTheme = {
    0x101820, 0x18242F, 0x0B1118, 0x26A69A, 0x90A4AE, 0xFFFFFF, 0x80CBC4,
};

constexpr ThemePalette kLightTheme = {
    0xF3F6F8, 0xFFFFFF, 0xDCE5EA, 0x00796B, 0x546E7A, 0x102027, 0x00695C,
};
...
void setSaveState(const char *text) {
    if (!saveStateValue) return;
    lv_label_set_text(saveStateValue, text);
}
...
void loadPersistentSettings() {
    ...
    Serial.printf("PREF LOAD: theme=%s brightness=%u\n",
...
void scheduleBrightnessSave() {
    ...
    setSaveState("Brightness changed - pending NVS save");
}
...
void serviceBrightnessSave() {
    ...
    setSaveState("Settings saved to NVS");
...
void buildHomePage() {
    ...
    makeTextLabel(contentArea, "Persistent LVGL settings on WT32-SC01-PLUS", 20, 54);
    makeMutedLabel(contentArea, "Theme and brightness survive restart through NVS.", 20, 84);

    lv_obj_t *card = makeCard(contentArea, 40, 122, 400, 96);
    lv_obj_t *label = lv_label_create(card);
    lv_label_set_text_fmt(label,
                          "Restored theme: %s\n"
                          "Restored brightness: %u%%",
                          themeName(), static_cast<unsigned>(currentBrightness));
...
void buildRemotePage() {
    ...
    makeMutedLabel(contentArea, "Persistent-settings compatibility page", 20, 50);
...
void onBrightness(lv_event_t *event) {
    ...
    currentBrightness = static_cast<uint8_t>(lv_slider_get_value(slider));
...
void buildSettingsPage() {
    ...
    makeTextLabel(contentArea, "Appearance", 20, 54);
    makeMutedLabel(contentArea, "Dark", 150, 54);

    lv_obj_t *themeSwitch = lv_switch_create(contentArea);
    lv_obj_set_pos(themeSwitch, 205, 47);
    ...
    lv_obj_set_style_bg_color(themeSwitch, lv_color_hex(theme().accent),
                              LV_PART_INDICATOR | LV_STATE_CHECKED);
    lv_obj_add_event_cb(themeSwitch, onThemeSwitch, LV_EVENT_VALUE_CHANGED, nullptr);

    makeMutedLabel(contentArea, "Light", 270, 54);

    makeTextLabel(contentArea, "Backlight", 20, 111);
    ...
    brightnessValue = makeLabel(contentArea, "", 405, 107, theme().accent);
    lv_label_set_text_fmt(brightnessValue, "%u%%", static_cast<unsigned>(currentBrightness));

    saveStateValue = makeMutedLabel(contentArea, "NVS settings loaded", 20, 166);
    makeMutedLabel(contentArea, "Brightness writes are delayed to reduce NVS wear.", 20, 195);
}
...
void buildInfoPage() {
    ...
    makeMutedLabel(contentArea, "Runtime + persistent UI state", 170, 14);
...
void setup() {
    Serial.begin(115200);
    delay(500);
    Serial.println("WT32-SC01-PLUS 17_LVGL_SettingsPersistence");
    ...
    Serial.printf("READY: settings restored theme=%s brightness=%u\n",
                  themeName(), static_cast<unsigned>(currentBrightness));
}
